...
- Admin exposes `GET|POST /api/servers`, `GET|PUT|DELETE /api/servers/<server_id>`, `POST /api/servers/<server_id>/test-ssh`, `POST /api/servers/<server_id>/preflight-node`, `POST /api/servers/<server_id>/inspect-node`, `POST /api/servers/<server_id>/onboard-node`, `POST /api/servers/<server_id>/deploy-node`, `POST /api/servers/<server_id>/restart-node`, `POST /api/servers/<server_id>/upgrade-node`, `POST /api/servers/<server_id>/rollback-node`, `GET /api/jobs`, `GET /api/jobs/<job_id>`, `POST /api/jobs/<job_id>/cancel`, and `GET /api/events`.
- SSH tests, node preflights, runtime inspections, node onboarding, deployments, restarts, upgrades, and rollbacks run as asynchronous jobs. The Servers tab shows the latest job status, structured steps, and logs; API clients can poll `/api/jobs/<job_id>` and cancel queued/running jobs with `POST /api/jobs/<job_id>/cancel`.
...
- Cancelling a job marks it `cancelled`, signals the running worker context, stops later steps from starting, and prevents a cancelled job from being overwritten as `succeeded`. Cancellation is not a transactional rollback of remote side effects that already completed; use `Inspect Node`, `Rollback Node`, or a fresh deploy to reconcile a partially changed VPS.
...
- Successful upgrades store the previous image as `previous_image`. Rollback jobs replace the compose `image:` line with `previous_image`, recreate the container, and then swap the current/previous image metadata so the rollback can be reversed if needed.
...
- Admin exposes `GET /api/domains`, `POST /api/domains`, `GET|PUT|DELETE /api/domains/<domain>`, `POST /api/domains/<domain>/onboarding-check`, `POST /api/domains/<domain>/onboarding-run`, `POST /api/domains/<domain>/dns-plan`, `POST /api/domains/<domain>/sync-dns`, `POST /api/domains/<domain>/failover-plan`, `POST /api/domains/<domain>/failover-run`, `POST /api/domains/<domain>/failover-reset`, `POST /api/domains/<domain>/cert-plan`, `POST /api/domains/<domain>/issue-cert`, and `POST /api/domains/<domain>/import-cert`.
...
- `POST /api/domains/<domain>/onboarding-check` returns a read-only checklist for production onboarding. It checks whether the domain is enabled, an active node is assigned, Cloudflare credentials are configured, the active node has a fresh healthy heartbeat and public IP, a certificate exists and is unexpired, assigned nodes have synced the certificate, failover has enough nodes when enabled, and Cloudflare DNS/certificate/failover plans can be built. The Domains tab exposes this as `Onboarding Check`.
- `POST /api/domains/<domain>/onboarding-run` starts an asynchronous domain onboarding job. The job can issue or renew the certificate, wait until selected/assigned nodes report the matching certificate version in heartbeat, sync Cloudflare DNS to the selected active node, optionally enable heartbeat failover, and then run a final readiness check. Poll `/api/jobs/<job_id>` for structured steps and logs; use `POST /api/jobs/<job_id>/cancel` or the Jobs table `Cancel` action to stop a queued/running onboarding job.
...
- In the Admin Domains tab, use `Onboarding Check` after saving the domain to see the same readiness evidence in one place. Use `Run Onboarding` for the recommended write path: issue cert, wait for node cert sync, sync DNS, and optionally enable automatic failover as one auditable job.
...
6. Run `Inspect Node` after onboarding or upgrades to confirm the remote compose image, node mode, node ID, central URL, container status, and published proxy ports match the intended deployment. Use `Rollback Node` if an upgrade needs to return to the previously recorded image.
...
8. In `Domains`, assign a domain to multiple nodes, choose the active node, run `Onboarding Check`, then use `Run Onboarding` to issue the certificate, wait for node certificate sync, sync Cloudflare DNS, and optionally enable failover.
9. Tune failover threshold and cooldown. Use manual lock when you want DNS pinned during maintenance.
10. Use the Jobs tab to inspect long-running deployment/domain tasks; cancel a queued or running job before changing credentials, domains, or image tags.
11. Use `Preview Failover` to confirm the current active node, target candidate, threshold, cooldown, and block reason. Use `Run Failover` for an immediate one-shot check instead of waiting for the next reconcile tick.
...
- Domain onboarding readiness test: call `/api/domains/<domain>/onboarding-check` and verify missing Cloudflare settings, missing/expired certificates, missing active node heartbeat, non-public IPs, and insufficient failover nodes are reported as blocking checks; verify a healthy active node with synced certificate and valid Cloudflare settings reports ready.
...
- Job cancellation test: start a queued/running provisioning or domain onboarding job, call `POST /api/jobs/<job_id>/cancel`, verify status becomes `cancelled`, polling stops as terminal, later steps do not start, and the job is not overwritten as `succeeded`.
...
- Rollback test: upgrade a test node to a new image, verify `previous_image` is recorded, call `/rollback-node`, and verify compose returns to the previous image while current/previous metadata is swapped.
